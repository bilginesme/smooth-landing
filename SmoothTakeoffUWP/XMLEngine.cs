...
namespace SmoothLandingUWP
{
    public class XMLEngine
    {
            ...
            if (fileContent != string.Empty)
            {
                ...
                var result = from c in xDoc.Descendants("Pomodoro")
                             select new
                             {
                                 ...
                                 IdleTimeHours = c.Element("IdleTimeHours").Value,
                                 IdleTimeMinutes = c.Element("IdleTimeMinutes").Value,
                                 IdleTimeSeconds = c.Element("IdleTimeSeconds").Value
                             };
                foreach (var el in result)
                {
                    ...
                    pomodoro.BackToLife(theDay, sliceNow, pomodorosToday, state,
                        Convert.ToInt16(el.Minutes), Convert.ToInt16(el.Seconds), 
                        Convert.ToInt16(el.IdleTimeHours), Convert.ToInt16(el.IdleTimeMinutes), Convert.ToInt16(el.IdleTimeSeconds));
                }
            }

            callback?.Invoke(pomodoro);
        }
        public static async Task<bool> UpdatePomodoroFile(Pomodoro pomodoro)
        {
            StringBuilder sb = CreatePomodoroXML(pomodoro);
            string strFileNameTemp = "pomodoro-tmp.dat";

            try
            {
                Windows.Storage.StorageFolder storageFolder = Windows.Storage.ApplicationData.Current.LocalFolder;
                Windows.Storage.StorageFile tempFile = await storageFolder.CreateFileAsync(strFileNameTemp, Windows.Storage.CreationCollisionOption.ReplaceExisting);

                await Windows.Storage.FileIO.WriteTextAsync(tempFile, sb.ToString());

                Windows.Storage.StorageFile oldFile = await storageFolder.GetFileAsync(fileNamePomodoro);
                if (oldFile != null)
                    await oldFile.DeleteAsync();

                await tempFile.RenameAsync(fileNamePomodoro);
            }
            catch (Exception e)
            {
        ...
        private static async Task<string> GetTextFromFile(FileTypeEnum fileType)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (fileType == FileTypeEnum.Pomodoro)
                        await UpdatePomodoroFile(new Pomodoro()).ConfigureAwait(false);
                    else if (fileType == FileTypeEnum.Statistics)
        ...
        private static async Task<bool> CreateNewPomodoroFileOBSOLETE()
        {
            Windows.Storage.StorageFolder storageFolder = Windows.Storage.ApplicationData.Current.LocalFolder;
        ...
        private static StringBuilder CreatePomodoroXML(Pomodoro pomodoro)
        {
            ...
            xmlWriter.WriteStartElement("IdleTimeHours");
            xmlWriter.WriteString(pomodoro.TSIdleTime.Hours.ToString());
            xmlWriter.WriteEndElement();

            xmlWriter.WriteStartElement("IdleTimeMinutes");
            xmlWriter.WriteString(pomodoro.TSIdleTime.Minutes.ToString());
            xmlWriter.WriteEndElement();

            xmlWriter.WriteStartElement("IdleTimeSeconds");
            xmlWriter.WriteString(pomodoro.TSIdleTime.Seconds.ToString());
            xmlWriter.WriteEndElement();
